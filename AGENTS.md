...
- **樣式**：`src/style.css`。
- **新精靈／圖示**：遵守 `.cursor/rules/pocket-pet-assets.mdc`；idle 見 `idleSpriteForSpeciesStage`（`src/pet.ts`）；照護姿勢檔名見 **`carePoseFile`**（`src/pet.ts`），由 `src/main.ts` 呼叫。**圖鑑**（`renderSpeciesDex`）除成長階段外另列四種照護姿勢示意（`DEX_POSE_STAGE`、狗用 `initDexDogCanvases` 的 `data-dex-pose`）。
