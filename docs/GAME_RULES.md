...
- **最低虛擬日齡**：未達 **12** 虛擬日前不判定進化。
- **分支優先序**（`pickMorphKey`，實作見 `src/pet.ts`）：
...
4. **對戰外觀與結算用快照**：開房與加入時，客戶端會上傳目前寵物的 **`species` / `nickname` / `virtAge` / `power`（訓練值，0～100）**；若已進化可選送 **`morphKey`**（`striker`／`guardian`／`survivor`／`harmony`）。伺服器在 **`linked`** 事件裡把對手的這組資料給另一方，對戰畫面用來顯示**正確精靈**、**對方暱稱**與形態字樣。`power` 會換算戰鬥 **MP 上限**（見第 2.5 節）；舊版客戶端未送 `power` 時伺服器以 **12** 視同；未送 `morphKey` 則視為無形態標記。
