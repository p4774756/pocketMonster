...
4. **對戰外觀與結算用快照**：開房與加入時，客戶端會上傳目前寵物的 **`species` / `nickname` / `virtAge` / `power`（訓練值，0～100）**；若已進化可選送 **`morphKey`**（`striker`／`guardian`／`survivor`／`harmony`／`cat_volt`／`cat_aqua`／`cat_flora`／`dog_volt`／`dog_aqua`／`dog_pyro`／`dog_tox`／`doodoo`）；並可附帶 **`playerTag`（對戰代號）** 供對方辨識（見下項）。伺服器在 **`linked`** 事件裡把對手的這組資料給另一方，對戰畫面用來顯示**正確精靈**、**對方暱稱**與形態字樣。`power` 會換算戰鬥 **MP 上限**（見第 2.5 節）；舊版客戶端未送 `power` 時伺服器以 **12** 視同；未送 `morphKey` 則視為無形態標記；未送或格式不符的 `playerTag` 則不帶給對方。
5. **對戰代號與本機好友備忘**：瀏覽器可自動產生一組 **對戰代號**（英數字元，不含易混淆字元），開房／加入時一併上傳。**非帳號系統**，不保證全域唯一，僅供約戰辨識。你可在大廳把代號加入 **好友備忘**（標籤名自訂，資料僅存本機）；`list_open_rooms` 回傳的公開房列若含房主代號，且與備忘相符，清單會加上「好友」提示。對戰結束時，若對方有提供代號且尚未在備忘中，可一鍵加入備忘。
