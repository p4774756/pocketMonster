...
2. 雙方連上後開始對戰；斷線、投降、結算規則由伺服器處理（見 `server/index.js`）。
3. **對戰外觀**：開房與加入時，客戶端會上傳目前寵物的 **`species` / `nickname` / `virtAge`**；伺服器在 **`linked`** 事件裡把對手的這組資料給另一方，對戰畫面用來顯示**正確精靈**與**對方暱稱**（與本機 HP 無關，僅展示）。
