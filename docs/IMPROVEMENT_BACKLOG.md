...
- **仍屬規劃／待辦**：形態專屬美術、戰鬥本場 log、`server/index.js` 模組化、大廳廣播聊天／代友照護等——見 **`ROADMAP_TASKS.md`** 未勾選項。**選用 Firebase 好友**（代碼加友、邀請、名單、**一對一文字聊天**；頂欄可**手動**雲端備份養成 JSON，**非**即時雲端養成；不含對戰 Socket）已見 **`docs/FIREBASE_FRIENDS.md`**。（根目錄 **README** 已補。）
