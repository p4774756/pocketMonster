...
**進度摘要（請隨版本更新此段）**：**v0.3 進化核心**已擴充：貓／狗屬性鍵、`cat_volt`／`cat_aqua` 專用 PNG、圖鑑改版、`doodoo` 大便怪 Canvas、狗 Canvas 四屬、`lightsOn` 與夜間結算（`GAME_RULES.md` §1.7～1.8）；**根目錄 README**（技術棧、啟動、部署、圖片示意）已補；**本機好友備忘／對戰代號**已自產品移除（改納入 v0.4「帳號制社交」長線）；**v0.3 其餘**（戰鬥 log、server 模組化、草貓專屬圖等）仍為待辦。
...
- [ ] **帳號制社交（大工程）**：可加好友、一對一或小型聊天、代友照護（須身分驗證、伺服器儲存好友關係／訊息／授權與寵物狀態同步策略）；與現行「匿名房間碼＋本機養成」架構分線規劃。
